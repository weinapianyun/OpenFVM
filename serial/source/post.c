...
PrintAsciiVectorElement (FILE * fp, int iphix, int iphiy, int iphiz)
{
  ...
  for (i = 0; i < nbelements; i++)
	...
	vx = V_GetCmp (&xu, element + 1);
      if (iphix == 1)
	vx = V_GetCmp (&xv, element + 1);
      if (iphix == 2)
	vx = V_GetCmp (&xw, element + 1);
      if (iphix == 3)
	vx = V_GetCmp (&xp, element + 1);
      if (iphix == 4)
	vx = V_GetCmp (&xT, element + 1);
      if (iphix == 5)
	vx = V_GetCmp (&xs, element + 1);

      if (iphiy == 0)
	vy = V_GetCmp (&xu, element + 1);
      if (iphiy == 1)
	vy = V_GetCmp (&xv, element + 1);
      if (iphiy == 2)
	vy = V_GetCmp (&xw, element + 1);
      if (iphiy == 3)
	vy = V_GetCmp (&xp, element + 1);
      if (iphiy == 4)
	vy = V_GetCmp (&xT, element + 1);
      if (iphiy == 5)
	vy = V_GetCmp (&xs, element + 1);

      if (iphiz == 0)
	vz = V_GetCmp (&xu, element + 1);
      if (iphiz == 1)
	vz = V_GetCmp (&xv, element + 1);
      if (iphiz == 2)
	vz = V_GetCmp (&xw, element + 1);
      if (iphiz == 3)
	vz = V_GetCmp (&xp, element + 1);
      if (iphiz == 4)
	vz = V_GetCmp (&xT, element + 1);
      if (iphiz == 5)
	vz = V_GetCmp (&xs, element + 1);
...
void
WriteAsciiVectorElement (FILE * fp, char *label,
...
void
WriteAsciiVectorMagnitudeFace (FILE * fp, char *label, int iphix,
			       int iphiy, int iphiz, double curtime)
{
...
void
WriteBinaryVectorMagnitudeFace (FILE * fp, char *label, int iphix,
				int iphiy, int iphiz, double curtime)
{
