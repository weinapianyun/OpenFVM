...
GradientX(Vector * phi, Vector * phif, int element ,double wf) // 最小二乘法 梯度计算方法
{
    ...
    Q_Constr (&G, "Left element", 3, False,Rowws,
              Normal, True);

    V_SetAllCmp (&g, 3, 0.0); // 将向量 g、h 初始化为 0.0
    V_SetAllCmp (&h, 3, 0.0);
    ...
    for (i = 0; i < elements[element].nbfaces; i++) // 遍历单元的所有界面
    {
        ...
        if (pair != -1) // 非边界界面
        {
            ...
            for (j = 0; j < 3; j++) // 给右端向量h 系数矩阵 G 赋值
            {
                ...
                V_SetCmp (&h, j+1, V_GetCmp(&h,j+1) + wf * wf * (V_GetCmp (phi, element + 1)
                                                                 - V_GetCmp (phi, neighbor + 1)) * df[j]);
                for (k = 0; k < 3; k++)
                    // G(i,j) = wf*wf*df(i)*df(j), 其中 i、j 为 x y z
                    Q_SetEntry (&G, j+1, k, k+1, Q_GetVal(&G, j+1, k)
                                                 + wf * wf * df[j] * df[k]);
            }
        }
        else // 边界界面
    ...
    // 求解变量的梯度值
    &g = MulInv_QV(&G, &h); // g = h/G , 即 g = G-1 * h
