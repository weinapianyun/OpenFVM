...
SetInitialConditions () // 设置初始条件
{
    ...
    int element, volume;
    int rv;
    double value; // 存储从字符串中读出的物理值

    gs = calloc (MAXL, sizeof (char)); // 过渡的内存空间
    for (i = 0; i < nbelements; i++) // 遍历网格单元
    {
        element = i;
        elements[element].bc = NONE; // 设置无边界条件
    ...
    for (j = 0; j < nbbcvolumes; j++) // 遍历边界条件的单元
    {
        volume = j;
        for (i = 0; i < nbelements; i++) // 遍历网格单元
        {
            ...
            {
                elements[element].bc = bcvolumes[volume].bc; // 设置对应的网格单元的边界条件类型
                ...
                // 设置边界单元的物理值
                strcpy (gs, bcvolumes[volume].fu); // 把 fu指向的字符串复制到 gs中
                strcat (gs, "\n"); // 把 "\n"追加到 gs 所指向的字符串的结尾
                rv = evaluate (gs, &value);
                V_SetCmp (&xu, element + 1, value); // 用 value值 初始化 xu
    ...
    for (i = 0; i < nbfaces; i++) // 遍历所有界面
    {
        ...
        if (pair != -1) // 若界面为内部界面
        {
            ...
            */
            ...
            V_SetCmp (&uf, face + 1,
                      (V_GetCmp (&xu, neighbor + 1) * lambda + V_GetCmp (&xu, element + 1) *
                                                               (1.0 - lambda)) * faces[face].n.x +(V_GetCmp (&xv, neighbor + 1) * lambda +
                                                                                                   V_GetCmp (&xv, element + 1) * (1.0 - lambda)) * faces[face].n.y +
                      (V_GetCmp (&xw, neighbor + 1) * lambda + V_GetCmp (&xw, element + 1) *
                                                               (1.0 - lambda)) * faces[face].n.z);
...
SetMaterialProperties () // 设置材料性质参数
{
    ...
    for (i = 0; i < nbelements; i++) // 遍历网格单元
    {
        ...
        // 便于设置初始时已充填的流场   fr1 = min{ max{ (0.5*F+0.5*F0), 0} ,1}
        fr[1] = LMIN(LMAX(V_GetCmp (&xs, element + 1) * 0.5 + V_GetCmp (&xs0, element + 1) * 0.5, 0.0), 1.0);
        fr[0] = 1.0 - fr[1];

        // 设置 材料的密度、粘度、比热容、热导率 (两相)
        V_SetCmp (&dens, element + 1,material.dens[0].cdens *
                                     fr[0] +material.dens[1].cdens * fr[1]);

        V_SetCmp (&visc, element + 1,material.visc[0].cvisc *
                                     fr[0] +material.visc[1].cvisc * fr[1]);

        V_SetCmp (&spheat, element + 1,material.therm[0].cspheat *
                                       fr[0] +material.therm[1].cspheat * fr[1]);

        V_SetCmp (&thcond, element + 1,material.therm[0].cthcond *
                                       fr[0] +material.therm[1].cthcond * fr[1]);
    }
}
