...
BuildContinuityMatrix (double dt) // 创建连续性方程的矩阵
{
    ...
    for (i = 0; i < nbelements; i++) // 遍历网格单元
    {
        ...
        for (j = 0; j < elements[element].nbfaces; j++) // 遍历每个网格单元的所有界面
        {
            ...
            else // 边界界面
            {
                ...
                if (faces[face].bc == PERMEABLE) // 渗透边界
                {
                    ...
                    // 将acn项 归入 bcp中,  求和[ -Af / (amf * |df|) * pf * (1-F) ]
                ...
                if (faces[face].bc == PRESSUREINLET) // 压力边界
                {
                    ...
                    {
                        bcp += 1.0 * parameter.orthof / (apj * (faces[face].dj + faces[face].kj)) *
                               (GeoDotVectorVector (gradpp, GeoSubVectorVector (faces[face].rpl,
                                                                                elements[element].celement)))* faces[face].Aj;
                    }
                }
                ...
                {
                    // pressure gradient = 0 , acp 、acn项为 0
                    // specified velocity
        ...
        Q_SetLen (&Ac, element + 1, nj + 1); // Am矩阵每行中 对角线右侧 系数的数目
    ...
    for (i = 0; i <= parameter.northocor; i++) // 依据 非正交修正次数 多次计算
    {
        ...
        if (pchecks == LOGICAL_TRUE) // 开启对计算参数的检查
        {
            if (!CheckIfDiagonalMatrix (&Ac)) // 检查 Ac 是否为 对角占优矩阵
            {
                ...
                WriteMatrix (&Ac, LOGICAL_TRUE); // 若不是，则将矩阵 Ac,向量 bp写入文件中
                WriteVector (&bp);
                //exit (LOGICAL_ERROR);
            }
        }
        ...
        // Solve matrix to get pressure p  求解连续性方程得到 压力场 P
        ...
        if (verbose == LOGICAL_TRUE) // 将presc作为第 i 次修正后的非正交误差, 在命令行中打印出来
